...
<%
	...
	FileBoardVO board = (FileBoardVO) request.getAttribute("board");
...
<body>
							...
							<td width="10%">
								<div align="center">
									<input type="image"
										...
										onclick="location.href='<%=contextPath%>/FileBoard/list.bo?currentBlock=<%=currentBlock%>&currentPage=<%=currentPage%>'" />
									&nbsp;
								</div>
							</td>
							<td width="42%"></td>
	...
	<form id="frmReply" action="<%=contextPath%>/FileBoard/reply.bo" method="post">
	...
	<script type="text/javascript">
		$("#pass").on("focusout", function() {
			...
			$.ajax({
				url: "<%=contextPath%>/FileBoard/password.bo",
				type: "post",
				async: true,
				data: {
					idx: <%=idx%>,
		...
		$("#update").click(function() {
			...
			$.ajax({
				url: "<%=contextPath%>/FileBoard/update.bo",
				type: "post",
				async: true,
				data: {
					email: $("#email").val(),
		...
		function deletePro(idx) {
			...
			if (window.confirm("게시글을 삭제하시겠습니까?")) {
				$.ajax({
					url: "<%=contextPath%>/FileBoard/delete.bo",
					type: "post",
					async: true,
					data: {
						idx: idx
