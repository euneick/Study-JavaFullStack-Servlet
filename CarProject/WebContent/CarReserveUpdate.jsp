...
<body>
	...
	<%
		...
		CarConfirmVO carConfirm = (CarConfirmVO) request.getAttribute("carConfirm");
		
		int carqty = carConfirm.getCarqty(); //대여수량
		int orderid = carConfirm.getOrderid(); //예약한 아이디
		String carimg = carConfirm.getCarimg(); //예약한 차량 이미지
		String carbegindate = carConfirm.getCarbegindate();	//대여시작날짜
		int carreserveday = carConfirm.getCarreserveday(); //대여기간
		int carins = carConfirm.getCarins(); //보험적용여부
		int carwifi = carConfirm.getCarwifi(); //wifi적용여부
		int carnave = carConfirm.getCarnave(); //네비적용여부
		int carbabyseat = carConfirm.getCarbabyseat(); //베이비시트적용여부	
	%>				
		<%-- 예약 정보를 수정 요청! --%>
		<form action="<%=contextPath%>/Car/ReserveUpdate.do?memberphone=${requestScope.memberphone}" method="post">
